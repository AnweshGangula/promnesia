...
import { _fmt, asClass, Binder } from "./display";
...
class Sidebar {
    ...
    setupFrame(
        ...
        promnesiaRootDiv: HTMLDivElement
    ): void {
        ...
        // I spent hours trying to implement it on pure css via vars/calc -- but didn't manage
        {
            ...
            if (
                values.every(
                    (v) => getComputedStyle(frame).getPropertyValue(v) == null
                )
            ) {
        ...
        promnesiaRootDiv.addEventListener(
            "click",
            defensify(async () => {
                await this.hide();
...
function _highlight(text: string, idx: number, v: Visit) {
    ...
    for (const line of text.split("\n")) {
        ...
        if (sline.length <= 3) {
            console.debug(
                "promnesia: line '%s' was completely sanitized/too short.. skipping",
                line
            );
            continue;
        }
    ...
    for (const [line, target_] of findMatches(doc.body!, lines)) {
        ...
        const ratio =
            (rect.width * rect.height) / (d.scrollWidth * d.scrollHeight);
        const RATIO = 0.5; // kinda arbitrary
        if (ratio > RATIO) {
            console.warn(
                "promnesia: matched element %o is too big (ratio %f > %f). skipping it",
                target,
                ratio,
                RATIO
            );
    ...
    for (const target of to_hl) {
        ...
        ref.title = `promnesia #${idx}: ${v.sources.join(" ")} ${(USE_ORIGINAL_TZ
            ? v.dt_local
            : v.time
        ).toLocaleString()}`;
...
function tryHighlight(text: string, idx: number, v: Visit) {
    ...
    try {
        ...
        console.error(
            "promnesia: while highlighting %s %o: %o",
            text,
            v,
            error
        );
...
async function* _bindSidebarData(response: Visits) {
    ...
    function shouldYield() {
        ...
        console.debug(
            "promnesia: bindSidebarData: %d iterations passed, took %o ms",
            iterations_since,
            took
        );
        last_yield = cur;
    ...
    for (const [tag_, count] of [
        [null, with_ctx.length],
        ...Array.from(all_tags).sort(),
    ]) {
        ...
        const tag_c = binder.makeChild(filtersEl, "span", [
            "src",
            "button",
            asClass(tag),
        ]);
    ...
    const visit_date_time = (v: Visit) =>
        _fmt(USE_ORIGINAL_TZ ? v.dt_local : v.time);
    ...
    for (const group of groups()) {
        ...
        const ctx =
            total_dur == null
                ? null
                : `Time spent: ${format_duration(total_dur)}`;
...
if (window.promnesia_haslistener == null) {
    ...
    console.debug(
        `[promnesia] [sidebar-${UUID}] skipping callback registration, they are already present`
    );
}
