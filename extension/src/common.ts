...
export async function fetch_max_stale(
    url: string,
    { max_stale }: { max_stale: number }
    ...
    const cached_resp = await fetch(url, { cache: "force-cache" }).then(
        rejectIfHttpError
    );
    ...
    const stale_ms =
        new Date().getTime() -
        new Date(expires == null ? 0 : expires).getTime();
